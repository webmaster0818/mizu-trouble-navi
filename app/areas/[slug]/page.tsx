    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <section className="mb-12">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 border-l-4 border-primary pl-4">{area.name}対応の水道修理業者5社比較</h2>
              ...
              <tbody>
                {companies.map((c) => (
                  <tr key={c.id} className="border-b hover:bg-blue-50">
                    ...
                    <td className="p-3"><span className="bg-green-100 text-success px-2 py-1 rounded text-xs font-bold">対応可</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* 各社紹介 */}
        <section className="mb-12">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 border-l-4 border-primary pl-4">各社の特徴</h2>
            ...
            {companies.map((c) => (
              <div key={c.id} className="bg-white border border-gray-200 rounded-lg p-5">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-accent font-bold text-sm">{c.rating}/5.0</span>
                </div>
                <p className="text-gray-600 text-sm mb-3">{c.description}</p>
                <div className="flex flex-wrap gap-2">
                  {c.features.slice(0, 3).map((f, i) => (
                    <span key={i} className="bg-primary-light text-primary text-xs px-2 py-1 rounded">{f}</span>
                  ))}
                </div>
              </div>
            ))}
        ...
        {/* エリアの特徴 */}
        <section className="mb-12">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 border-l-4 border-primary pl-4">{area.name}の水道事情</h2>
        ...
        {/* 緊急連絡先 */}
        <section className="mb-12">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 border-l-4 border-accent pl-4">緊急連絡先</h2>
          <div className="bg-accent-light border border-accent/20 rounded-lg p-6">
            <div className="space-y-3 text-sm">
              <div>
                ...
                <p className="text-accent font-bold text-lg">{area.waterBureauPhone}</p>
              </div>
        ...
        <section className="mt-12">
          <h2 className="text-xl font-bold text-gray-900 mb-4">その他のエリア</h2>
          <div className="flex flex-wrap gap-2">
            {areas.filter(a => a.slug !== slug).slice(0, 15).map((a) => (
              <Link key={a.slug} href={`/areas/${a.slug}/`} className="px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs hover:border-primary hover:text-primary transition">
                {a.name}
              </Link>
            ))}
          </div>
