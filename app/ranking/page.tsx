  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">水道修理業者おすすめランキング5選</h1>
            ...
            <tbody>
              {companies.map((c) => (
                <tr key={c.id} className="border-b border-gray-200 hover:bg-blue-50">
                  ...
                  <td className="p-3">
                    <span className="text-accent font-bold">{c.rating}</span>/5.0
                  </td>
                  <td className="p-3 text-accent font-bold">{c.basePrice}</td>
        ...
        {/* 各社詳細レビュー */}
        {companies.map((c) => (
          <section key={c.id} className="mb-12 bg-white border border-gray-200 rounded-lg p-6 md:p-8">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-accent font-bold text-lg">{c.rating}</span>
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <dl className="space-y-1 text-sm">
                  <div className="flex justify-between"><dt className="text-gray-600">基本料金</dt><dd className="font-bold text-accent">{c.basePrice}</dd></div>
        ...
        <div className="text-center mt-8">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href="/comparison/price/" className="px-4 py-2 border border-primary text-primary rounded-lg text-sm hover:bg-primary hover:text-white transition">料金比較</Link>
            <Link href="/comparison/speed/" className="px-4 py-2 border border-primary text-primary rounded-lg text-sm hover:bg-primary hover:text-white transition">スピード比較</Link>
            <Link href="/comparison/warranty/" className="px-4 py-2 border border-primary text-primary rounded-lg text-sm hover:bg-primary hover:text-white transition">保証比較</Link>
            <Link href="/comparison/reviews/" className="px-4 py-2 border border-primary text-primary rounded-lg text-sm hover:bg-primary hover:text-white transition">口コミ比較</Link>
          </div>
        </div>
      </div>
      <CTASection />
